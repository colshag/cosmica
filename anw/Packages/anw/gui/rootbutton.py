...
class RootButton(DirectObject):
    ...
    def createInfoPane(self, text, wordwrap, x, z, scale=0.025, textColor=globals.colors['white']):
        ...
        myInfoPane.setCardColor(globals.colors['guiblue3'])
        myInfoPane.setFrameColor(globals.colors['guiblue2'])
        myInfoPane.setFrameAsMargin(.3, .5, .5, .5)
        myInfoPane.setFrameLineWidth(3)
        myInfoPane.setCardAsMargin(.3, .5, .5, .5)
