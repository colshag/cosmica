...
class DialogBox(RootButton):
    """The Dialog Box Gui"""
    def __init__(self, path, x=0, y=0, text='Insert Dialog Text Here',textColor=globals.colors['orange']):
        RootButton.__init__(self, path, x=x, y=y, name='okiunderstand',ignoreShortcutButtons=[],createButtons=False)
        self.scale = 0.25
        height = self.createInfoPane(text, wordwrap=50,x=x-0.25,z=y, scale=0.04, textColor=textColor)
